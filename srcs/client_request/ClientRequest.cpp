...
void	ClientRequest::setPath(const std::string path)
{
	...
	int i = 0;
	std::string tmp ;
	serverClass &serv = getConfigServerRef(getPort());
	std::map<std::string, std::string>::iterator itdata= serv.getBeginDataIterator();
	std::map<std::string, std::string>::iterator itedata= serv.getEndDataIterator();

	while (itdata != itedata)
	{
		if (itdata->first == "root")
		{
			// std::cout << "setting path to " << it->second << std::endl;
			i = 1;
			break;
		}
		itdata++;
...
int	ClientRequest::root_modify()
{
	serverClass &tmp = getConfigServerRef(getPort());
	std::map<std::string, std::string>::iterator it = tmp.getBeginDataIterator();
	std::map<std::string, std::string>::iterator ite = tmp.getEndDataIterator();
